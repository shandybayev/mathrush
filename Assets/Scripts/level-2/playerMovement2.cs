...
public class playerMovement2 : MonoBehaviour {
    ...
    void Start()
    {
        ...
        my_object = FindObjectOfType<Equations2>();
        my_object.setUp();
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.name == "finish")
        {
            ...
            if (Equations2.my_score >= 70)
            {
                next_level.SetActive(true);

                level_controller.isOpen3 = true;
                PlayerPrefs.SetInt("open3", level_controller.isOpen3 ? 1 : 0);
            }
            else
                next_level.SetActive(false);
        }
    ...
    {
        timer_end = FindObjectOfType<Timer2>();
